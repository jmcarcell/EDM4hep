...
void processEvent(podio::EventStore& store, bool verboser, unsigned eventNum) {
  ...
  if( tpchs.isValid() ){
    ...
    for(int j=0 ; j<ntpch ; ++j){
      ...
      if ( tpch1.getTime() !=  j*3.f )  throw std::runtime_error("time != j*3") ;
      ...
      if ( tpch2.getTime() !=  j*3.f )  throw std::runtime_error("time != j*3") ;
 ...
 if( thps.isValid() ){
   ...
   for(int j=0 ; j<nthp ; ++j){
     ...
     if ( thp1.getTime() !=  j*3.f )  throw std::runtime_error("time != j*3") ;
     if ( thp1.getEDep() !=  j * 0.000001f )  throw std::runtime_error("eDep != j * 0.000001") ;
     if( !( thp1.getPosition() == edm4hep::Vector3d( j*2.f , j*3.f , j*5.f  )) )
     ...
     if ( thp2.getTime() !=  j*3.f )  throw std::runtime_error("time != j*3") ;
     if ( thp2.getEDep() !=  j * 0.0000031f )  throw std::runtime_error("eDep != j * 0.0000031 ") ;
     if( !( thp2.getPosition() == edm4hep::Vector3d( -j*2.f , -j*3.f , -j*5.f  )) )
          throw std::runtime_error("position != ( -j*2. , -j*3. , -j*5. )") ;
   }
 }else {
